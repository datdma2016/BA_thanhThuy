...
<style>
    ...
    .dot { color: #58a6ff; } /* Màu xanh cho dấu chấm */
...
def home():
    return f"""
    <h1>Bot V33: Chatterbox Mode (Alive Signal)</h1>
    <ul>
...
def lay_data_tong_hop():
    def generate():
        yield CSS_STYLE
        yield "<div class='log warning'>Vui lòng dùng /fb-daily.</div>"
...
def lay_data_hang_ngay():
    def generate():
        yield CSS_STYLE
        yield "<h3>> KHỞI ĐỘNG BÁO CÁO NGÀY (V33 - ALIVE SIGNAL)...</h3>"
        try:
            ...
            if not start_date_str or not end_date_str:
                yield "<div class='log error'>[LỖI] Thiếu start hoặc end (YYYY-MM-DD)!</div>"
                return

            yield f"<div class='log info'>[INIT] File='{FILE_SHEET_GOC}' | Tab='{ten_tab}'</div>"
...
# ======================================================
# PROCESS DAY-BY-DAY (V33 - BẮN TÍN HIỆU LIÊN TỤC)
# ======================================================
def process_day_by_day(keyword, ten_tab, start_date_str, end_date_str):
    ...
    for single_date in daterange(start_dt, end_dt):
        ...
        for i, tk_obj in enumerate(DANH_SACH_TKQC):
            ...
            # --- ĐIỂM KHÁC BIỆT: IN TRƯỚC KHI QUÉT ---
            ...
            # --- VÒNG LẶP LẤY DỮ LIỆU (BẮN TÍN HIỆU LIÊN TỤC) ---
            while True:
                try:
                    # Bắn tín hiệu "tao còn sống"
                    yield "<span class='dot'>.</span>" 
                    yield "<script>window.scrollTo(0, document.body.scrollHeight);</script>"
                    ...
                    yield f"<div class='log error'>[API FAIL] {str(e)}</div>"
                    break
        ...
        if BUFFER_ROWS:
            ...
            yield f"<br><div class='log info'>Ngày {current_date_str} Empty.</div>"
        
        yield f"<div class='log sleep'>⏳ Waiting: </div>"
    ...
    yield f"<div class='log success' style='margin-top:20px; border-top:2px solid #3fb950; padding-top:10px;'>🎉🎉🎉 XONG!</div>"
